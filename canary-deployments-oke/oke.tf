...
resource "null_resource" "oke_nginix_setup" {
  ...
  depends_on = [module.oci-oke]
   ...
   provisioner "local-exec" {
      ...
      command = "echo '(1) Kubeconfig setup: '; export KUBECONFIG=${path.module}/generated/kubeconfig;kubectl create ns ${var.deploy_stage_canary_namespace};kubectl create ns  ${var.deploy_stage_prd_namespace}"
