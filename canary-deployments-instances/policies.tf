...
resource "oci_identity_dynamic_group" "runcmddynamicgroup" {
  ...
  compartment_id = var.tenancy_ocid
  matching_rule  = "All {instance.id = '${oci_core_instance.compute_instance_prod.id}',instance.id = '${oci_core_instance.compute_instance_canary.id}'}"
}
...
resource "oci_identity_policy" "devopspolicy" {
  ...
  statements = [
   "Allow group Administrators to manage devops-family in compartment id ${var.compartment_ocid}",
    ...
    "Allow dynamic-group ${oci_identity_dynamic_group.devopsgroup1.name} to use instance-agent-command-execution-family in compartment id ${var.compartment_ocid} where request.instance.id='${oci_core_instance.compute_instance_prod.id}'",
    "Allow dynamic-group ${oci_identity_dynamic_group.devopsgroup1.name} to use instance-agent-command-execution-family in compartment id ${var.compartment_ocid} where request.instance.id='${oci_core_instance.compute_instance_canary.id}'",
    ...
    "Allow dynamic-group ${oci_identity_dynamic_group.runcmddynamicgroup.name} to use instance-agent-command-execution-family in compartment id ${var.compartment_ocid} where request.instance.id='${oci_core_instance.compute_instance_prod.id}'",
    "Allow dynamic-group ${oci_identity_dynamic_group.runcmddynamicgroup.name} to use instance-agent-command-execution-family in compartment id ${var.compartment_ocid} where request.instance.id='${oci_core_instance.compute_instance_canary.id}'",
